    ...
    this.createKnob = function(width, height) {
      ...
      const knob = {
        ...
        'redraw': function() {
          ...
          const labelSize = Math.round(0.6 * lineWidth);
...
function demoKnob(gauge_id, label, valMin, valMax, color_scheme, initValue=0) {
  ...
  knob.setProperty('colorFG', color_scheme);
  knob.setProperty('colorLabel', color_scheme);
  ...
  // Add it to the DOM.
  const elem = document.getElementById(gauge_id);
  elem.knob = knob;
...
function update(range = false) {
  ...
  xmlhttp.open("GET", window.location.origin + "/get_latest", true);
  xmlhttp.send();
